...
import { fetchQuestions } from '../actions/verbActions';
    ...
    evaluateAnswer = answer => {
        ...
        if(this.props.questions.length > 0) {
            ...
            if(answer.toLowerCase().replace(/[.,\/#!?$%\^&\*;:{}=\-_`~()]/g,"").trim() === correctAnswer) {
                alert("si eso es correcto :)");
                this.props.incrementCorrectAnswers(); 
            } else {
                alert("lo siento por favor intenta nuevamente :(");
        ...
        if(this.props.questions.length === 0) {
            return (
                <div className="Questions-Loading">
                    <img src={spinner} />
                    <p>En el proceso de descarga de preguntas ...</p>
                </div>
            )
        } else {
            return (
